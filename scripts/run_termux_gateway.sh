#!/usr/bin/env bash
#
# Termux USB Gateway Launcher (`scripts/run_termux_gateway.sh`)
#
# Usage:
#   ./scripts/run_termux_gateway.sh        -> Auto-detects USB device (if only 1 connected)
#   ./scripts/run_termux_gateway.sh 2      -> Selects /dev/bus/usb/001/002
#   ./scripts/run_termux_gateway.sh 002    -> Selects /dev/bus/usb/001/002
#
...
if arg:
    ...
    if len(devices) == 1:
        ...
        print(f'ERROR: Multiple USB devices found: {devices}. Please specify the number (e.g. ./run_termux_gateway.sh 2)', file=sys.stderr)
        sys.exit(1)
")"

...
class TestBatchSerialization(unittest.TestCase):
    ...
    def test_10_sample_batch_reconstruction(self):
        """Simulate packing 10 samples and verifying timestamp reconstruction."""
        device_id = b"NODE_3A\x00"
        start_ts_ms = 1000000 # 1000 seconds
        interval_ms = 1000    # 1 second spacing
        sample_count = 10
        status = 0x4D4F
        vbat_mv = 3300

        hdr_bytes = struct.pack(self.batch_hdr_fmt, device_id, start_ts_ms, interval_ms, sample_count, status, vbat_mv)
        ...
        # Unpack header
        dev_id_out, start_ts_out, interval_out, count_out, status_out, vbat_out = struct.unpack(self.batch_hdr_fmt, full_payload[:19])
        self.assertEqual(dev_id_out.decode('utf-8').rstrip('\x00'), "NODE_3A")
        self.assertEqual(start_ts_out, start_ts_ms)
        ...
        for i in range(count_out):
            ...
            sample_ts_us = (start_ts_out + (i * interval_out)) * 1000
            
            self.assertEqual((x, y, z), expected_samples[i])
            self.assertEqual(sample_ts_us, (1000000 + (i * 1000)) * 1000)
